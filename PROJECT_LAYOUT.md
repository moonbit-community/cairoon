...
- 153 `*_test.mbt` files under `src/tests/`, including black-box and oracle
  packages.
...
| White-box oracles | `src/tests/oracle/<family>/` plus shared C support in `src/tests/oracle/native/` | Import `CAIMEOX/cairoon` for the public API and declare test-only direct-C oracle externs locally; public binding wrappers must never import oracle packages. Test-only C symbols are provided by the oracle-native support package, not `src/moon.pkg`. The image pattern-stack dispatcher remains routing-only while scene ranges 34-36 and 37-41 live in focused sibling test files. |
...
6. **Family package probes**: started. Primitive Cairo constants now live in
   ...
   state. It owns `RawDevice` and all 17 Device, Surface-get-device,
   recording-surface, and script-surface externs. No Device extern remains in
   the public package. Public `Device`
   ...
   `RawContext` and all 109 externs: 16 core, 11 clip/extents, 19
   font/text, 11 matrix, 14 paint, 17 path, and 21 state declarations. Public
   `Context` remains an abstract single-field wrapper with no second finalizer;
   no Context FFI bridge remains in the root; typed facade values are converted
   to raw handles, arrays, and integers before child-package calls. The child
   constructs valid contexts from `RawSurface` and `RawMappedImageSurface`
...
- `moon.mod` registers `scripts/build/cairo_config.py`, every native-stub
  package uses exactly
  `"${build.CAIRO_CFLAGS} -std=c11 -Wall -Wextra -Wpedantic -Werror"`, and no
  package or consumer stores concrete Cairo paths or `cc-link-flags`;
