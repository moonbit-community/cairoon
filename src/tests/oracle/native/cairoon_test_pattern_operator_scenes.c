...
cairo_status_t cairoon_test_apply_radial_hsl_color_surface_mask_combo(
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_surface_set_device_offset(surface, -0.25, -0.20);
    status = cairo_surface_status(surface);
  }
  ...
  cairo_pattern_t *radial = NULL;
  if (status == CAIRO_STATUS_SUCCESS) {
    radial = cairo_pattern_create_radial(3.0, 13.0, 0.5, 13.5, 2.5, 8.0);
    status = cairo_pattern_status(radial);
  }
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_pattern_add_color_stop_rgba(radial, 0.0, 0.10, 0.86, 0.94, 0.84);
    cairo_pattern_add_color_stop_rgba(radial, 0.36, 0.95, 0.24, 0.18, 0.78);
    cairo_pattern_add_color_stop_rgba(radial, 0.78, 0.24, 0.94, 0.28, 0.72);
    cairo_pattern_add_color_stop_rgba(radial, 1.0, 0.88, 0.86, 0.16, 0.86);
    cairo_matrix_t matrix;
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_pattern_add_color_stop_rgba(mask, 0.0, 1.0, 1.0, 1.0, 0.12);
    cairo_pattern_add_color_stop_rgba(mask, 0.52, 1.0, 1.0, 1.0, 0.76);
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_set_source_rgb(cr, 0.026, 0.030, 0.050);
    cairo_paint(cr);
    cairo_save(cr);
    cairo_rectangle(cr, 1.0, 1.0, width - 2.0, height - 2.0);
    cairo_clip(cr);
    ...
    cairo_set_source(cr, radial);
    cairo_move_to(cr, 2.0, 3.0);
    cairo_curve_to(cr, 5.5, 0.5, 12.5, 2.0, width - 2.0, 7.0);
    cairo_line_to(cr, width - 4.0, height - 2.0);
    cairo_curve_to(cr, 8.5, 12.5, 4.5, 14.5, 2.0, height - 5.0);
    cairo_close_path(cr);
    cairo_fill(cr);
    cairo_set_operator(cr, CAIRO_OPERATOR_OVER);
    cairo_set_source_rgba(cr, 0.95, 0.95, 0.88, 0.12);
    cairo_rectangle(cr, 3.0, 11.0, width - 6.0, 2.5);
    cairo_fill(cr);
    cairo_pop_group_to_source(cr);
    cairo_mask(cr, mask);
    cairo_restore(cr);
...
cairo_status_t cairoon_test_apply_linear_hsl_luminosity_surface_mask_combo(
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_surface_set_device_offset(surface, 0.10, -0.45);
    status = cairo_surface_status(surface);
  }
  ...
  cairo_pattern_t *linear = NULL;
  if (status == CAIRO_STATUS_SUCCESS) {
    linear = cairo_pattern_create_linear(0.0, height - 2.0, width, 1.0);
    status = cairo_pattern_status(linear);
  }
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_pattern_add_color_stop_rgba(linear, 0.0, 0.94, 0.78, 0.12, 0.82);
    cairo_pattern_add_color_stop_rgba(linear, 0.34, 0.18, 0.92, 0.86, 0.74);
    cairo_pattern_add_color_stop_rgba(linear, 0.66, 0.92, 0.20, 0.34, 0.78);
    cairo_pattern_add_color_stop_rgba(linear, 1.0, 0.24, 0.30, 0.96, 0.88);
    cairo_matrix_t matrix;
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_pattern_add_color_stop_rgba(mask, 0.0, 1.0, 1.0, 1.0, 0.16);
    cairo_pattern_add_color_stop_rgba(mask, 0.48, 1.0, 1.0, 1.0, 0.84);
    cairo_pattern_add_color_stop_rgba(mask, 1.0, 1.0, 1.0, 1.0, 0.94);
    cairo_matrix_t matrix;
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_set_source_rgb(cr, 0.018, 0.024, 0.046);
    cairo_paint(cr);
    cairo_save(cr);
    cairo_rectangle(cr, 1.0, 1.0, width - 2.0, height - 2.0);
    cairo_clip(cr);
    ...
    cairo_set_source(cr, linear);
    cairo_move_to(cr, 1.5, height * 0.50);
    cairo_curve_to(cr, 4.0, 1.0, 10.5, 2.0, width - 1.5, 5.5);
    cairo_line_to(cr, width - 3.0, height - 2.5);
    cairo_curve_to(
      cr,
      8.5,
      height - 1.0,
    ...
    cairo_fill(cr);
    cairo_set_operator(cr, CAIRO_OPERATOR_OVER);
    cairo_set_source_rgba(cr, 1.0, 0.96, 0.72, 0.14);
    cairo_set_line_width(cr, 1.25);
    cairo_move_to(cr, 3.0, 4.0);
    cairo_curve_to(cr, 6.0, 9.5, 10.0, 5.5, width - 3.0, height - 4.5);
    cairo_stroke(cr);
    cairo_pop_group_to_source(cr);
    cairo_mask(cr, mask);
    cairo_restore(cr);
