...
cairo_status_t cairoon_test_render_vector_scene_oracle(
  ...
  if (scene == CAIROON_TEST_VECTOR_BACKEND_NESTED_TAGS) {
    return cairoon_test_render_backend_nested_tag_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_PAGE_SEQUENCE) {
    return cairoon_test_render_backend_page_sequence_features(kind, name);
  }
  ...
  if (scene == CAIROON_TEST_VECTOR_BACKEND_METADATA_OUTLINE) {
    return cairoon_test_render_backend_metadata_outline_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_PAGE_OPS) {
    return cairoon_test_render_backend_page_ops_features(kind, name);
  }
  ...
  if (scene == CAIROON_TEST_VECTOR_BACKEND_OUTLINE_SEQUENCE) {
    return cairoon_test_render_backend_outline_sequence_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_PATTERN_TAG) {
    return cairoon_test_render_backend_pattern_tag_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_ANNOTATION_SEQUENCE) {
    return cairoon_test_render_backend_annotation_sequence_features(kind, name);
  }
  ...
  if (scene == CAIROON_TEST_VECTOR_BACKEND_REVIEW_DOSSIER) {
    return cairoon_test_render_backend_review_dossier_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_APPENDIX_RUBRIC) {
    return cairoon_test_render_backend_appendix_rubric_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_RESEARCH_NOTE) {
    return cairoon_test_render_backend_research_note_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_CROSS_REFERENCE) {
    return cairoon_test_render_backend_cross_reference_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_LINK_AUDIT) {
    return cairoon_test_render_backend_link_audit_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_ATTACHMENT_INDEX) {
    return cairoon_test_render_backend_attachment_index_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_NAVIGATION_MAP) {
    return cairoon_test_render_backend_navigation_map_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_ACCESSIBILITY_INDEX) {
    return cairoon_test_render_backend_accessibility_index_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_SECTION_CATALOG) {
    return cairoon_test_render_backend_section_catalog_features(kind, name);
  }
  if (scene == CAIROON_TEST_VECTOR_BACKEND_TAG_ATTRIBUTES) {
    return cairoon_test_render_backend_tag_attribute_features(kind, name);
  }
