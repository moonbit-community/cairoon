...
static cairo_status_t cairoon_test_configure_backend_page_combo(
  cairo_surface_t *surface,
  int32_t kind) {
  switch (kind) {
    case CAIROON_TEST_VECTOR_PDF:
      ...
      cairoon_test_pdf_surface_set_custom_metadata(
        ...
        "Combo",
        "surface-page-tags");
      cairo_pdf_surface_set_page_label(surface, "page-copy-cover");
      cairo_pdf_surface_add_outline(
        surface,
        CAIRO_PDF_OUTLINE_ROOT,
        "page copy cover",
    ...
    case CAIROON_TEST_VECTOR_PS:
      ...
      cairo_ps_surface_dsc_comment(
        surface,
        "%%Title: Cairoon Surface Page Combo");
      cairo_ps_surface_dsc_comment(
        surface,
        "%%IncludeFeature: *MediaColor White");
      cairo_ps_surface_dsc_begin_page_setup(surface);
...
static cairo_status_t cairoon_test_draw_backend_page_combo(
  cairo_surface_t *surface,
  cairo_t *cr,
  int32_t kind) {
  cairo_select_font_face(
  ...
  cairo_set_source_rgb(cr, 0.9, 0.93, 1.0);
  cairo_paint(cr);
  cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
  cairo_tag_begin(
    cr,
    CAIRO_TAG_LINK,
    "uri='https://example.com/page-combo'");
  cairo_move_to(cr, 10.0, 20.0);
  cairo_show_text(cr, "surface page combo");
  cairo_tag_end(cr, CAIRO_TAG_LINK);
  ...
  if (status == CAIRO_STATUS_SUCCESS && kind == CAIROON_TEST_VECTOR_PDF) {
#if CAIRO_HAS_PDF_SURFACE && CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 15, 10)
    cairo_pdf_surface_set_page_label(surface, "page-copy-retained");
    cairo_pdf_surface_add_outline(
      surface,
      CAIRO_PDF_OUTLINE_ROOT,
      "page copy retained",
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    cairo_set_source_rgba(cr, 0.85, 0.1, 0.1, 0.72);
    cairo_tag_begin(cr, CAIRO_TAG_LINK, "dest='page-combo-dest'");
    cairo_rectangle(cr, 10.0, 36.0, 44.0, 20.0);
    cairo_fill(cr);
    cairo_tag_end(cr, CAIRO_TAG_LINK);
  ...
  if (status == CAIRO_STATUS_SUCCESS && kind == CAIROON_TEST_VECTOR_PDF) {
#if CAIRO_HAS_PDF_SURFACE && CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 15, 10)
    cairo_pdf_surface_set_page_label(surface, "page-copy-final");
    cairo_pdf_surface_add_outline(
      surface,
      CAIRO_PDF_OUTLINE_ROOT,
      "page copy final",
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    ...
    cairo_tag_begin(cr, CAIRO_TAG_DEST, "name='page-combo-dest' x=10 y=62");
    cairo_move_to(cr, 10.0, 62.0);
...
static cairo_status_t cairoon_test_render_backend_page_combo_surface(
  cairo_surface_t *surface,
  int32_t kind) {
  cairo_status_t status = cairo_surface_status(surface);
  if (status == CAIRO_STATUS_SUCCESS) {
    status = cairoon_test_configure_backend_page_combo(surface, kind);
  }
  ...
  if (status == CAIRO_STATUS_SUCCESS) {
    status = cairoon_test_draw_backend_page_combo(surface, cr, kind);
  }
...
#if CAIRO_HAS_PDF_SURFACE && CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 15, 10)
      surface = cairo_pdf_surface_create(name, 100.0, 100.0);
      return cairoon_test_render_backend_page_combo_surface(surface, kind);
#else
      return CAIRO_STATUS_INVALID_STATUS;
#endif
    case CAIROON_TEST_VECTOR_PS:
