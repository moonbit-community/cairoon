...
| Reliability ledger | `API_INVENTORY.md` statuses are `Done`, `Partial`, or `Decision`; every `Partial` row names its remaining gap; this scorecard and CI/verify gate are checked by `scripts/check-reliability-ledger.py` | Exact for current migrated slices; 33 behavior regressions are split across dedicated hosted-workflow, downstream/verify, current release-evidence, and exact local-matrix test modules, each below the 600-line Python-test budget. Hosted policy and restricted YAML parsing are separate implementation modules. Three root-aware delegation tests pin the CLI over focused Markdown, release-evidence, shell/matrix, and hosted-workflow implementations. The one remaining `Partial` row names shipped release-platform evidence, and the full-product claim still requires it to reach `Done` or an explicit scope `Decision` |
| FFI boundary safety | Production raw `src/**/ffi*.mbt` declarations are native-gated in their owning `moon.pkg`, mark every non-primitive C FFI parameter with `#borrow` or `#owned`, and `scripts/check-project-layout.py`, `scripts/check-ffi-ownership.py`, `scripts/check-external-owners.py`, plus `scripts/check-stream-cleanup.py` pass | Strong for current raw externs, including internal helper packages; the ownership gates enforce Device, Surface, mapped-image, stream-constructor, and borrowed-return cleanup/reference rules plus scoped-error precedence. The 344-line FFI CLI owns declaration/facade/export parity while focused `scripts/ffi_ownership/` modules own shared parsing, family cleanup, and borrowed-return rules; eight guard tests pin all seven root-aware delegations, six reference helpers, nine producer paths, raster callback ingress, contract deletion, dead-branch rejection, and main-path wiring. The 258-line layout CLI owns path assembly and output while focused `scripts/project_layout/` modules own parsing, metadata/docs/consumer, native/package, and counter rules; eighteen tests include all eleven root-aware delegations. All 34 production and 51 oracle C stubs compile under the exact C11 `-Wall -Wextra -Wpedantic -Werror` contract, and the layout gate rejects any downgrade. The external-owner gate independently discovers all 12 raw owner declarations, native finalizers, and external-object allocators; enforces structural owner naming; and requires exact release plus top-level 1000-iteration lifetime evidence |
...
`scripts/check-ffi-ownership.py` whenever raw extern declarations or borrowed
Cairo object producers change; its semantic and call-site ledgers reject removed
contracts, missing references, wrong base owners, duplicate producers,
owned-wrapper drift, and disabled or nested success-path evidence. Run
...
`scripts/verify.sh`.
The image oracle keeps pattern-stack scene ids stable through a routing-only
dispatcher; scene bodies 34-36 and 37-41 live in separate focused files while
both ordinary and buffer-backed surfaces continue to use the same direct C
oracle comparison.
...
- 226/226 repository script tests pass.
...
- All 12 raw external owners have exact finalizer and 1000-iteration stress
  evidence; all 6 borrowed-reference helpers and 9 borrowed producer paths have
  fail-closed static evidence; every package passes ASan/LSan/UBSan locally.
- Source, extracted-archive, and unmodified cross-host consumers each pass all
  six public workflows; all 695 publication members match source bytes.
...
- Hosted workflow structure is locally fail-closed: runner labels, dependency
  installs, MoonBit setup, generated constants, step order, and gate commands
  must exactly match the audited `ubuntu-24.04`/`macos-15` contract.
