...
comparisons for eighteen deterministic ARGB32 image scenes on ordinary and
...
rendering, dashed round-cap strokes, and clipped paint/fill output;
...
- ASan/LSan via `run-asan.py`: ran the 108-test native suite after the PNG
  ...
  The later image oracle slices added private C helpers for eighteen deterministic
  ...
  raster-source pattern repeat rendering, dashed round-cap stroke state, and
  clipped paint/fill output, without changing the current test count because
  they broaden one existing white-box oracle test. ASan/LSan
