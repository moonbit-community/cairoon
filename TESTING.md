...
scenes, two PDF-only document-feature oracle scenes including one text/tag-aware
...
destination and document-structure rectangle/text tag oracle scenes, two PDF
...
document-feature, and PS/SVG Link/destination/document-structure direct-oracle
coverage, and the remaining API families from `API_INVENTORY.md`.
