...
comparisons for twenty-three deterministic ARGB32 image scenes on ordinary and
...
`OperatorClear` compositing output, group compositing output, and a
surface-pattern `Reflect`/`Nearest`/`DitherBest`/matrix combination plus
transformed repeated linear-gradient `Repeat`/`Nearest`/`DitherFast`/matrix
output and mesh-pattern fill output;
