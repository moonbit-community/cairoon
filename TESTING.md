...
| Publication integrity | The zip contains the declared dual-license notice and full texts, native-only module metadata, package README/interface, dependency pre-build script, pinned pycairo API snapshot, and source-identical external-owner checker/ledger; canonical paths are unique, safe, CRC-valid, and exclude integration fixtures | Done locally: `scripts/check-publication-archive.py` checks all 629 members, and thirteen focused tests cover valid archives plus missing/incorrect license data, metadata/target mismatch, missing or altered build/API/owner support, duplicate names, unsafe paths, fixture leakage, emptiness, and corruption |
...
Exact Linux Cairo 1.15.10 and 1.18.4 each pass 829/829 native tests, 180/180
