...
stringification, PNG filename load/save for image surfaces,
...
covers empty, single-rectangle, and multi-rectangle
...
- ASan/LSan via `run-asan.py`: ran the 108-test native suite after the PNG
  filename API slice and failed in LeakSanitizer. The leak report is rooted
  in
  ...
  Summary: `91317 byte(s) leaked in 1299 allocation(s)`.
