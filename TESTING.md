...
wide three-page marker checks, two
...
- `./scripts/verify.sh`: passed. The local reliability gate ran
  ...
  stream equivalence slice, and the PDF JPEG MIME stream equivalence slice.
- `moon -C cairoon check --target native`: passed.
- `moon -C cairoon test --target native`: 416 tests passed. The current run
...
- `moon -C cairoon test surface_stream_test.mbt --target native -v`: 10
  ...
  mapping.
- `moon -C cairoon test surface_stream_wbtest.mbt --target native -v`: 14
  ...
  labels/outlines/tags, PS DSC, and SVG document units, plus PDF JPEG MIME and
...
- `MOON_CC=/opt/homebrew/opt/llvm/bin/clang MOON_AR=/usr/bin/ar
  ASAN_OPTIONS=detect_leaks=0:fast_unwind_on_malloc=0 moon -C cairoon test
  surface_stream_wbtest.mbt --target native -v`: 14 ASan-compiled white-box
  ...
  paths, plus glyph_path/show_glyphs vector scenes and the PDF JPEG MIME and
  PDF thumbnail stream-vs-file paths.
- `MOON_CC=/opt/homebrew/opt/llvm/bin/clang MOON_AR=/usr/bin/ar
  ASAN_OPTIONS=detect_leaks=0:fast_unwind_on_malloc=0 moon -C cairoon test
...
cross-backend tagged multi-page text oracle scene, one cross-backend tagged
  `show_text_glyphs` oracle scene, one cross-backend grouped glyph/tag
  multi-page oracle scene, one cross-backend copy_page retained two-page oracle
  scene, one cross-backend mixed
...
clip/dash/surface-pattern/mask/tag/text oracle scene, one cross-backend wide
  three-page URI/destination/document-structure tag/vector oracle scene, PDF
  tagged multi-page
text, tagged `show_text_glyphs`, grouped glyph/tag multi-page, copy_page
retained two-page,
mixed vector/tag/text, layered three-page, and wide three-page marker tests,
PS/SVG tag metadata absence checks, two PDF document-feature
...
layered three-page
clip/dash/surface-pattern/mask/tag/text, wide three-page tag/vector, PDF JPEG
MIME and PDF thumbnail output, and backend document-feature equality checks,
plus PDF/PS/SVG, PNG-writer, and
...
three-page marker tests, PS/SVG tag metadata absence checks, and
