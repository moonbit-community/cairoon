...
| PDF surface | Partial | Filename/no-output/stream-writer constructors, supported versions, version strings, version restriction, page size, metadata, custom metadata, page labels, thumbnails, single-flag outline compatibility, typed outline flag sets, stable structural output markers, direct C Cairo oracle comparison for seven deterministic PDF 1.4 vector scenes covering paint, stroke, fill/stroke rectangles, Bezier paths, transforms, and linear/radial gradients, PDF link-tag annotation markers, and stream callback error mapping; broader multi-page/font/tag/metadata differential output comparison remains |
| PS surface | Partial | Filename/no-output/stream-writer constructors, supported levels, level strings, level restriction, EPS mode, page size, DSC comments, stable page/drawing output markers, direct C Cairo oracle comparison for seven deterministic vector scenes covering paint, stroke, fill/stroke rectangles, Bezier paths, transforms, and linear/radial gradients with `CreationDate` normalization, Link tag inertness on PS output, and stream callback output/error mapping; broader multi-page/font/tag/metadata differential output comparison remains |
| SVG surface | Partial | Filename/no-output/stream-writer constructors, supported versions, version strings, version restriction, document units, stable geometry/color output markers, exact direct C Cairo oracle comparison for seven deterministic vector scenes covering paint, stroke, fill/stroke rectangles, Bezier paths, transforms, and linear/radial gradients, Link tag inertness on SVG output, and stream callback output/error mapping; broader multi-page/font/tag/metadata differential output comparison remains |
...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks plus direct C Cairo oracle comparisons for seven deterministic PDF/PS/SVG scenes, PDF link tags have annotation marker checks, PS/SVG link tags have inert-output checks, PDF/PS/SVG, PNG, and script stream callbacks have output/error coverage, PNG stream reading has image round-trip and short-read coverage, image/mapped-image data has mutable-view lifetime and unmap coverage, and image rendering has an eight-scene direct C Cairo ARGB32 oracle comparison, but broader normalized/differential comparison remains |
...
| `PDFSurface` | Partial | Exposed as `Surface::pdf`, `Surface::pdf_stream`, `PDFVersion::supported`, `PDFVersion::to_string`, `Surface::pdf_restrict_to_version`, size, metadata, custom metadata, page label, thumbnail, single-flag outline compatibility, and typed outline flag sets. Filename/no-output/stream constructors, PDF structural markers, seven-scene direct C oracle comparison for deterministic PDF 1.4 vector output, PDF 1.4 link-tag annotation markers, and stream `WriteError` mapping are covered; broader multi-page/font/tag/metadata differential PDF output tests remain. |
| `PSSurface` | Partial | Exposed as `Surface::ps`, `Surface::ps_stream`, `PSLevel::supported`, `PSLevel::to_string`, `Surface::ps_restrict_to_level`, EPS get/set, size, and DSC helpers. Filename/no-output/stream constructors plus PS page/drawing markers, seven-scene direct C oracle comparison with `CreationDate` normalization, Link tag inertness checks, and stream `WriteError` mapping are covered; broader multi-page/font/tag/metadata differential PS output tests remain. |
| `SVGSurface` | Partial | Exposed as `Surface::svg`, `Surface::svg_stream`, `SVGVersion::supported`, `SVGVersion::to_string`, `Surface::svg_restrict_to_version`, and document-unit get/set. Filename/no-output/stream constructors plus SVG geometry/color markers, exact seven-scene direct C oracle comparison, Link tag inertness checks, and stream output tests are covered; broader multi-page/font/tag/metadata differential SVG output tests remain. |
