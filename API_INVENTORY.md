...
| `RasterSourcePattern` | Partial | Exposed as `Pattern::raster_source`, compatible `raster_set_acquire`/`raster_get_acquire`/`raster_clear_acquire`, and pycairo-style optional `raster_set_callbacks`/`raster_get_callbacks`/`raster_clear_callbacks` for acquire-only, acquire+release, release-only, and clear states; C callback trampolines retain MoonBit closures and acquired surface wrappers, and tests cover paint output, release callbacks, release-only state, get-acquire callback identity/optional/clear behavior, data-backed source lifetime, retained-owner stress, subtype mismatch, finished-surface acquire failure injection, and 1000-iteration callback allocation stress covering set/get/manual acquire/release/replace/clear paths. Broader failure-injection/fuzz and differential rendering remain. |
