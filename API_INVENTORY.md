...
| `ScaledFont` | Partial | Constructor, status, equality/hash, extents, font face/options, font matrix, ctm, scale matrix, text extents, glyph extents, full text-to-glyph run, and glyph-only text conversion implemented; tests compare font/text/glyph extents against a direct C Cairo oracle. Broader text-to-glyph differential coverage and clean ASan remain |
