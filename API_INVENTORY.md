...
| Image surface basics | Partial | Create, similar-image create, map/unmap to image, buffer-backed create-for-data, PNG path load/save, MIME data, status, finish, flush, width/height/stride/format, copied data, mutable `ImageData` views for image and mapped-image surfaces, and one direct C Cairo oracle pixel comparison |
...
| PDF surface | Partial | Filename/no-output/stream-writer constructors, supported versions, version strings, version restriction, page size, metadata, custom metadata, page labels, thumbnails, single-flag outline compatibility, typed outline flag sets, stable structural output markers, PDF link-tag annotation markers, and stream callback error mapping; full normalized vector-output comparison remains |
...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks, PDF link tags have annotation marker checks, PDF/PS/SVG stream writer callbacks have output/error coverage, image/mapped-image data has mutable-view lifetime and unmap coverage, and image painting has an initial direct C Cairo oracle comparison, but full normalized/differential comparison remains |
...
| `ImageSurface` | Partial | Basic creation, buffer-backed creation, PNG path loading, copied readback, and retained mutable `ImageData` view |
...
| `ImageSurface.create_from_png/write_to_png` | Partial | Filename APIs exposed as `Surface::image_from_png` and `Surface::write_to_png`; PNG stream/file-object callbacks remain deferred by `AGENTS.md` |
