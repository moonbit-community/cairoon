...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks, PDF link tags have annotation marker checks, PDF/PS/SVG and PNG stream writer callbacks have output/error coverage, image/mapped-image data has mutable-view lifetime and unmap coverage, and image painting has an initial direct C Cairo oracle comparison, but full normalized/differential comparison remains |
...
| `ImageSurface.create_from_png/write_to_png` | Partial | Filename APIs exposed as `Surface::image_from_png` and `Surface::write_to_png`; PNG stream writes are exposed as `Surface::write_to_png_stream` with copied-byte writer callbacks and `WriteError` mapping; PNG read stream/file-object callbacks remain deferred by `AGENTS.md` |
