...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks, PDF link tags have annotation marker checks, image/mapped-image data has mutable-view lifetime and unmap coverage, and image painting has an initial direct C Cairo oracle comparison, but full normalized/differential comparison remains |
