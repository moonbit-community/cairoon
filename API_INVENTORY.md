...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks plus direct C Cairo oracle comparisons for deterministic simple PDF/PS/SVG paint fixtures, PDF link tags have annotation marker checks, PS/SVG link tags have inert-output checks, PDF/PS/SVG, PNG, and script stream callbacks have output/error coverage, PNG stream reading has image round-trip and short-read coverage, image/mapped-image data has mutable-view lifetime and unmap coverage, and image rendering has an eight-scene direct C Cairo ARGB32 oracle comparison, but broader normalized/differential comparison remains |
