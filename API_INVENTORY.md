...
| `PDFSurface` | Partial | Exposed as `Surface::pdf`, `Surface::pdf_stream`, `PDFVersion::supported`, `PDFVersion::to_string`, `Surface::pdf_restrict_to_version`, size, metadata, custom metadata, page label, thumbnail, single-flag outline compatibility, and typed outline flag sets. Filename/no-output/stream constructors, executable backend reference docs, PDF structural markers, metadata/custom-metadata/page-label/outline output markers, multi-page output markers, ten-scene direct C oracle comparison for deterministic PDF 1.4 vector output including toy-font text paths, toy-font `show_text`, and a two-page paint scene, PDF JPEG MIME payload embedding, PDF 1.4 URI link-tag annotation markers, named-destination tag markers, document-structure tag markers, stream `WriteError` mapping, and 1000-iteration stream callback allocation stress are covered; broader tag-aware metadata/multi-page PDF output tests remain. |
