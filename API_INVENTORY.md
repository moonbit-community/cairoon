...
| Tests | Partial | Native and ASan test runs cover the current migrated slices only; vector backends have stable structural output marker checks, PDF link tags have annotation marker checks, PDF/PS/SVG, PNG, and script stream callbacks have output/error coverage, PNG stream reading has image round-trip and short-read coverage, image/mapped-image data has mutable-view lifetime and unmap coverage, and image painting has an initial direct C Cairo oracle comparison, but full normalized/differential comparison remains |
