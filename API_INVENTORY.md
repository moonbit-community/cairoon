...
| Text and fonts | Partial | font options get/set, font face get/set/select, toy font, font matrix/size/extents, Context toy text APIs, Context glyph arrays, and scaled font basics exist; text-to-glyphs and `show_text_glyphs` remain |
...
| `ScaledFont` | Partial | Constructor, status, equality/hash, extents, font face/options, font matrix, ctm, scale matrix, text extents, and glyph extents implemented; text-to-glyphs remains |
| `Glyph` arrays | Partial | `ArrayView[Glyph]` marshaling exists for `Context::glyph_extents`, `Context::glyph_path`, `Context::show_glyphs`, and `ScaledFont::glyph_extents`; text-to-glyphs output marshaling remains |
