...
| `ImageSurface.create_for_data` | Partial | Exposed as `Surface::image_for_data` for `FixedArray[Byte]`; retains the MoonBit buffer, validates size/stride, covers zero-size buffers, shares storage with `ImageData`, and has thirteen-scene direct C Cairo ARGB32 oracle coverage for the backing-buffer path. Clean ASan/LSan remains blocked by the macOS font/text leak report |
