...
| Device basics | Done | External object ownership, status/type/equal/hash, finish/flush/acquire/release, scoped acquire helper, `Surface::get_device`, script file/stream devices, script-backed tests, and executable backend reference docs |
...
| Context basics | Partial | Creation, status, pointer equality/hash, save/restore, CTM transforms, drawing state including hairline mode, clip APIs, group APIs, tag APIs, path primitives, source colors/surfaces, paint/fill/stroke, and executable reference docs |
...
| Font options | Done | External object, copy/merge/equal/hash, all portable state fields, Surface/Context accessors, and executable Font reference docs |
...
| `Path` | Done | Opaque owned `cairo_path_t *`, status/equality/hash, stringification, segment values, iteration, and executable reference docs |
...
| `ScaledFont` | Partial | Constructor, status, equality/hash, extents, font face/options, font matrix, ctm, scale matrix, text extents, glyph extents, full text-to-glyph run, and glyph-only text conversion implemented; tests compare font/text/glyph extents plus empty, single/multi/spaced ASCII, and UTF-8 text-to-glyph coordinate cases against direct C Cairo oracles, and cover sheared font/CTM scale-matrix composition, embedded-NUL rejection for text extents, spaced ASCII cluster splitting, and glyph-only parity through public APIs. Broader shaping coverage and clean ASan remain |
...
| pycairo API | cairoon status | Notes |
|---|---|---|
| `Region` | Done | External object, empty/single/multi-rectangle constructors, copy, extents, rectangle access, predicates, equality, translate, and executable reference docs |
