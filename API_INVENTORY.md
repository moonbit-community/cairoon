...
| `ScaledFont` | Partial | Constructor, status, equality/hash, extents, font face/options, font matrix, ctm, scale matrix, text extents, glyph extents, full text-to-glyph run, and glyph-only text conversion implemented; tests compare font/text/glyph extents and a full text-to-glyph run against a direct C Cairo oracle. Broader font/string matrix coverage and clean ASan remain |
